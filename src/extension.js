...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("fontBigger.openStyleFile", async () => {
      ...
      const scriptPath = getPlotScriptPath(workspaceFolder);
      await ensureFileExists(scriptPath, `Plot script not found: ${scriptPath}`);
  ...
  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument(async (document) => {
      ...
      const plotScriptPath = getPlotScriptPath(workspaceFolder);
      const savedScript = samePath(document.uri.fsPath, plotScriptPath);

      if (!savedScript) {
        return;
      }

      await schedulePipeline(context, output, state, "plot script saved", workspaceFolder);
    })
  );
}
...
async function runPipelineOnce(context, workspaceFolder, output, progress) {
  ...
  await runPlotStep(workspaceFolder, config, plotScriptPath, output);

  progress.report({ message: "Compiling LaTeX..." });
  await runLatexStep(config, mainTexPath, output);
}

async function runPlotStep(workspaceFolder, config, plotScriptPath, output) {
  ...
  const args = [plotScriptPath];
  output.appendLine(`> ${config.pythonCommand} ${args.map(quoteShell).join(" ")}`);
...
async function runLatexStep(config, mainTexPath, output) {
  ...
  const compileBackend = resolveCompileBackend(config);
  ...
  if (compileBackend === "latex-workshop") {
    output.appendLine("> VS Code command: latex-workshop.build");
    await runEditorBuildCommand("latex-workshop.build", output, "LaTeX Workshop");
    return;
  }

  if (compileBackend === "overleaf-workshop") {
    output.appendLine("> VS Code command: overleaf-workshop.compileManager.compile");
    await runEditorBuildCommand(
      "overleaf-workshop.compileManager.compile",
      output,
      "Overleaf Workshop"
    );
    return;
...
function getConfig(workspaceFolder) {
  ...
  return {
    ...
    compileBackend: config.get("compileBackend", "auto"),
    runOnSave: config.get("runOnSave", true)
  };
}

function resolveCompileBackend(config) {
  if (config.compileBackend && config.compileBackend !== "auto") {
    return config.compileBackend;
  }

  if (hasExtension("james-yu.latex-workshop")) {
    return "latex-workshop";
  }

  if (hasExtension("iamhyc.overleaf-workshop")) {
    return "overleaf-workshop";
  }
...
async function runEditorBuildCommand(commandId, output, extensionName) {
  try {
    await vscode.commands.executeCommand(commandId);
  } catch (error) {
    throw new Error(
      `${extensionName} build failed. Check the ${extensionName} output channel. ${error.message}`
    );
  }
}

function getPlotScriptPath(workspaceFolder) {
  const config = getConfig(workspaceFolder);
  return resolveWorkspacePath(workspaceFolder, config.plotScript);
}
...
async function openConfigForFigureUnderCursor(workspaceFolder, output) {
  ...
  const scriptPath = resolveWorkspacePath(workspaceFolder, selectedFigure.entry.script);
  const document = await vscode.workspace.openTextDocument(scriptPath);
  await vscode.window.showTextDocument(document, {
    preview: false,
    viewColumn: vscode.ViewColumn.Beside
...
async function pickFigureFromRegistry(workspaceFolder, figures) {
  const items = Object.entries(figures).map(([id, entry]) => ({
    ...
    detail: Array.isArray(entry.outputs) ? entry.outputs.join(", ") : "",
    id,
    entry
  }));
...
async function activateFigure(workspaceFolder, figureId, entry) {
  if (!entry.script) {
    throw new Error(`Figure "${figureId}" must define "script" in the registry.`);
  }

  await updateWorkspaceConfiguration(workspaceFolder, resolveWorkspacePath(workspaceFolder, entry.script));
}
...
async function createCompatibleCopy(context, workspaceFolder, output) {
  ...
  if (await pathExists(compatibleScriptPath)) {
    ...
    if (action === "Open Existing") {
      await updateWorkspaceConfiguration(workspaceFolder, compatibleScriptPath);
      await openGeneratedFiles(compatibleScriptPath);
      return;
    }
  }
...
function generateCompatibleCopySource(originalSource, sourceRelativePath) {
...
def resolve_output_dir(style: dict = STYLE) -> Path:
...
async function updateWorkspaceConfiguration(workspaceFolder, compatibleScriptPath) {
  const config = vscode.workspace.getConfiguration("fontBigger", workspaceFolder.uri);
  await config.update(
    "plotScript",
...
async function openGeneratedFiles(scriptPath) {
